...
if(!empty($_POST) && $_POST['Action']=='login_form'){
    ...
    $email = safe_input($connect, $_POST['Email']);
    $password = safe_input($connect, $_POST['Password']);
    ...
    $result = mysqli_query($connect, "SELECT * FROM tbl_users WHERE email='$email' AND password='".md5($password)."' LIMIT 1");
...
if(!empty($_POST) && $_POST['Action']=='registration_form'){
    ...
    $name = safe_input($connect, $_POST['Name']);
    $email = safe_input($connect, $_POST['Email']);
    $password = safe_input($connect, $_POST['Password']);
    ...
    $result = mysqli_query($connect, "SELECT * FROM tbl_users WHERE email='$email' LIMIT 1");
    if(mysqli_num_rows($result)==1){
        ...
        mysqli_query($connect, "INSERT INTO tbl_users (user_GUID, email, password, entry_date) values(MD5(UUID()), '$email', '".md5($password)."' ,NOW() )");
        $user_id = mysqli_insert_id($connect); /* Get the auto generated id used in the last query */
        /*Insert registration data to user profile table */
        mysqli_query($connect, "INSERT INTO `tbl_user_profile` (user_id,name) VALUES('$user_id','$name')");
        /* Success: Set session variables and redirect to Protected page */
